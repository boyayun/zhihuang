...
#define ZHIHUANG_STATE_HEAD 0
#define ZHIHUANG_STATE_LEN  1
#define ZHIHUANG_STATE_DATA 2

#define ZHIHUANG_BUF_LEN  256
/**
   ...
   @param len 本次处理的数据长度
     @retval none
*/
extern unsigned char ZhihuangUnpack(unsigned char* des, unsigned char* src,unsigned char len);
/**
	 @brief 串口数据链路层组包并解包后存起来
   @attention 串口收到数据后，调用该接口
   @param data 本次处理的数据指针
...
/**
   ...
   @param srclen 本次处理的数据长度
     @retval 打包后的字节长度
*/
extern unsigned char zhiHuangPack(unsigned char* des,unsigned char*src,unsigned char srclen);
/**
	 @brief 串口数据链路层组包并打包后通过物理层发送出去
   @attention 将需要发送的数据组包
   @param des 接收打包后的数据的指针
