...
unsigned char ZhihuangUnpack(unsigned char* des, unsigned char* src,unsigned char len)
{
    unsigned char i;
    unsigned char checkvalue;           // 校验值
    unsigned char data_pos=0;           // 数据位置
    static unsigned char zhihuang_state = ZHIHUANG_STATE_HEAD;
    static unsigned char data[ZHIHUANG_BUF_LEN] = {0};
    static unsigned char data_len = 0;
    static bool rev_heading = false;        // 正在接收头
    ...
    for(i = 0;i < len;i++)
    {
        ...
        {
            if((src[i] == 0xAA) && (true == rev_heading))
            {
                zhihuang_state = ZHIHUANG_STATE_LEN;
                memset(data, 0, ZHIHUANG_BUF_LEN);
                data_pos = 0;
                data_len = 0;
                checkvalue = 0;
                rev_heading = false;
        ...
        if(ZHIHUANG_STATE_LEN == zhihuang_state)
        {
            data_len = src[i]-1;        // 减去校验长度
            zhihuang_state = ZHIHUANG_STATE_DATA;
        }
        else if(ZHIHUANG_STATE_DATA == zhihuang_state)
        {
            ...
            {
                zhihuang_state = ZHIHUANG_STATE_HEAD;
                if(checkvalue == src[i])
                {
                    rev_heading = false;
...
unsigned char zhiHuangPack(unsigned char* des,unsigned char*src,unsigned char srclen)
{
    unsigned char j = 0;
    ...
    des[3+srclen] = checkvalue;
    return srclen+4;    //2头1len1校验
}
